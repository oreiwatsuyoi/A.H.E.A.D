...
class ComplianceManager {
    ...
    logEvent(event, data) {
        ...
        if (database && myId) {
            database.ref('audit-logs').push(logEntry).catch(err => {
    ...
    generateComplianceReport() {
        return {
            ...
            auditTrail: this.sessionLogs.map(log => ({
                timestamp: log.timestamp || Date.now(),
                event: log.event || 'unknown',
                userId: log.userId || 'unknown',
                sessionId: log.sessionId || 'unknown',
                data: log.data || {},
                ipAddress: log.ipAddress || 'masked',
...
function endCallWithCompliance() {
    try {
        ...
        if (database && myId) {
            database.ref('compliance-reports').child(myId).set(report).catch(err => {
                console.warn('Failed to save compliance report:', err);
            });
        }
    } catch (error) {
...
window.addEventListener('unhandledrejection', function(event) {
    ...
    compliance.logEvent('UNHANDLED_REJECTION', { 
